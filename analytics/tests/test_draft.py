...
def test_recommends_the_player_you_would_lose_not_the_best_one():
    ...
    out = recommend(board, pick=20, next_pick=30, roster=[], config=CONFIG_OBJ, teams=10)
...
def test_a_player_already_on_your_own_roster_is_never_recommended_again():
    ...
    out = recommend(board, pick=20, next_pick=30, roster=roster, config=CONFIG_OBJ, teams=10)
...
def test_missing_p_gone_by_next_raises_a_clear_error_naming_add_survival():
    ...
    with pytest.raises(ValueError, match="add_survival"):
        recommend(board, pick=20, next_pick=30, roster=[], config=CONFIG_OBJ, teams=10)
...
def test_n_limits_the_number_of_rows_returned():
    ...
    out = recommend(board, pick=20, next_pick=30, roster=[], config=CONFIG_OBJ, teams=10, n=3)
    assert len(out) == 3
